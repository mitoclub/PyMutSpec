...
def main(path_to_obs, path_to_exp, outdir, outgrp, mut_num_for_192, image_extension):
    path_to_united_exp = os.path.join(outdir, "mean_expexted_mutations.tsv")
    path_to_ms12 = os.path.join(outdir, "ms12{}.tsv")
    path_to_ms12plot = os.path.join(outdir, "ms12{}." + image_extension)
    path_to_ms192plot = os.path.join(outdir, "ms192{}.") + image_extension
    ...
    for lbl_code in LABELS:
        ...
        if cur_obs.shape[0]:
            ...
            plot_mutspec12(ms12, title=f"{lbl} mutational spectrum",
                           savepath=path_to_ms12plot.format(lbl), show=False)
            if cur_obs.shape[0] > mut_num_for_192:
                ...
                plot_mutspec192(ms192, title=f"{lbl} mutational spectrum",
                                filepath=path_to_ms192plot.format(lbl), show=False)
